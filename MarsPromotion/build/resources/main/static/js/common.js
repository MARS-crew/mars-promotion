...
const jockeyType = document.querySelectorAll("[name='applyInfoType']");
...
if (applyBtn) {
  applyBtn.addEventListener("click", () => {
    ...
    generationValue = applyTypeValue.substring(0, applyTypeValue.length - 1);
    popupOpen();
    fetch("/api/v1/apply", {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        ...
        objective: generationValue,
        generation: jockeyTypeValue,
        reason: Motive.value,
        say: lastSay.value,
      }),
    }).then((response) => {
